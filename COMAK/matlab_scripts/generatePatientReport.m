function generatePatientReport(patientID, patientReportPath, display_names, patientLinks)
    % Generate an individual report for a patient
    fid = fopen(patientReportPath, 'w');
    if fid == -1
    ...
    resultsDir = '../results';
    info = dir(fullfile(resultsDir, patientID));
    ...
    % Write HTML content
    fprintf(fid, '<html>\n<head>\n<title>COMAK Simulation Results for %s</title>\n', patientID);
    fprintf(fid, '<style>\n');
    ...
    fprintf(fid, '  <a href="#Joint_Contact_Pressure">Joint Contact Pressure and Contact Area</a>\n');
    ...
    fprintf(fid, '<h1 style="margin: 0; font-size: 2.5em;">COMAK Simulation Results for %s</h1>\n', patientID);
    fprintf(fid, '<p style="font-size: 1.2em;">Report generated on: %s</p>\n', formattedDateTime);
    fprintf(fid, '</div>\n');

    % Joint Contact Pressure Section
    fprintf(fid, '<section id="Joint_Contact_Pressure">\n');
    fprintf(fid, '<h2 style="font-size: 2em; color: #333; margin-bottom: 10px;">Joint Contact Pressure</h2>\n');
    
    % Start of flexbox container for side-by-side images
    fprintf(fid, '<div class="gif-container">\n');
    
    % Side View GIF
    fprintf(fid, '<div style="text-align: center; margin: 10px;">\n');
    fprintf(fid, '<h3 id="Side_View" style="font-size: 1.5em; color: #444;">Joint Contact Pressure (Side View)</h3>\n');
    fprintf(fid, '<img class="sync-gif" src="../results/%s/graphics/paraview/side_animation_%s.gif" alt="Side View Animation" style="max-width:100%%; height:auto;">\n', patientID, patientID);
    fprintf(fid, '</div>\n');
    
    % Top View GIF
    fprintf(fid, '<div style="text-align: center; margin: 10px;">\n');
    fprintf(fid, '<h3 id="Top_View" style="font-size: 1.5em; color: #444;">Joint Contact Pressure (Top View)</h3>\n');
    fprintf(fid, '<img class="sync-gif" src="../results/%s/graphics/paraview/top_animation_%s.gif" alt="Top View Animation" style="max-width:100%%; height:auto;">\n', patientID, patientID);
    fprintf(fid, '</div>\n');
    ...
    fprintf(fid, '<div style="text-align: left; margin: 10px;">\n');  % Main container
    fprintf(fid, '<h3 style="font-size: 1.5em; color: #444; margin-top: 50px; margin-bottom: 10px; margin-left: 200px;">Contact Pressure  and Area Plot</h3>\n');  % Subtitle with left margin
    fprintf(fid, '<p style="font-size: 1.2em; color: #666; margin-left: 200px;">The following plot shows the contact area and pressure over the gait cycle. A vertical line loops along the time axis (gait cycle axis) at the same speed as the GIFs.</p>\n');  % Text with left margin
    fprintf(fid, '</div>\n');  % Close the left-margin div
    
    % Plot without left margin
    fprintf(fid, '<img class="sync-gif" src="../results/%s/graphics/paraview/%s_animated_joint_mechanics.gif" alt="Contact Area and Pressure Plot" style="max-width:100%%; height:auto;">\n', patientID, patientID);
    fprintf(fid, '</section>\n');
    ...
    metrics = {'Center_of_Pressure', 'Contact_Forces', 'Reaction_Moments'};
    ...
    fprintf(fid, '<img id="jointMechanicsPlot" src="../results/%s/graphics/extended_joint_mechanics/Center_Of_Pressure_Anterior-Posterior_plot.png" alt="Joint Mechanics Plot" style="width: 1240px; height:auto;">\n', patientID);
    fprintf(fid, '</div>\n');
    ...
    fprintf(fid, '  img.src = "../results/%s/graphics/extended_joint_mechanics/" + metric + "_" + dimension + "_plot.png";\n', patientID);
    fprintf(fid, '}\n');
    fprintf(fid, '</script>\n');
    ...
    fprintf(fid, '<p>Select the joint and degree of freedom to view the corresponding kinematics plot:</p>\n');
    ...
    fprintf(fid, '<img id="kinematicsPlot" src="../results/%s/graphics/kinematics/tibiofemoral_rotations.png" alt="Kinematics Plot" style="max-width:100%%; height:auto;">\n', patientID);
    fprintf(fid, '</div>\n');
    fprintf(fid, '</section>\n');
    ...
    fprintf(fid, '  img.src = "../results/%s/graphics/kinematics/" + joint + "_" + dof + ".png";\n', patientID);
    fprintf(fid, '}\n');
    fprintf(fid, '</script>\n');
    ...
    fprintf(fid, '<p>This plot shows the primary tibiofemoral kinematics along with motion capture data for validation:</p>\n');
    fprintf(fid, '<div style="text-align:center; margin-top: 20px;">\n');
    fprintf(fid, '<img src="../results/%s/graphics/validation/primary_tibiofemoral_kinematics_with_mocap_1.png" alt="Validation Plot" style="max-width:100%%; height:auto;">\n', patientID);
    fprintf(fid, '<img src="../results/%s/graphics/validation/primary_tibiofemoral_kinematics_with_mocap_2.png" alt="Validation Plot" style="max-width:100%%; height:auto;">\n', patientID);
    fprintf(fid, '</div>\n');
    fprintf(fid, '</section>\n');
    ...
    fprintf(fid, '<select id="muscleSelect" onchange="updatePlot(''muscleSelect'', ''musclePlot'', ''../results/%s/graphics/muscle_activations/'');">\n', patientID);
    ...
    fprintf(fid, '<div style="text-align:center;">\n');
    fprintf(fid, '<img id="musclePlot" src="../results/%s/graphics/muscle_activations/%s.png" alt="Muscle Activation Plot" style="max-width:100%%; height:auto;">\n', patientID, muscles{1});
    fprintf(fid, '</div>\n');
    ...
    fprintf(fid, '<select id="reserveSelect" onchange="updatePlot(''reserveSelect'', ''reservePlot'', ''../results/%s/graphics/reserve_actuators/'');">\n', patientID);
    ...
    fprintf(fid, '<div style="text-align:center;">\n');
    fprintf(fid, '<img id="reservePlot" src="../results/%s/graphics/reserve_actuators/%s.png" alt="Reserve Actuator Plot" style="max-width:100%%; height:auto;">\n', patientID, actuators{1});
    fprintf(fid, '</div>\n');
    ...
    if patientID(1) == 'S'
        ...
        fprintf(fid, '<p>Qualitative comparison assessing similarity in trends between simulated activations and EMG signals using Cross-Correlation to measure timing and shape similarity.</p>\n');
        ...
        fprintf(fid, '<div style="text-align:center; margin-top: 20px;">\n');
        fprintf(fid, '<img src="../results/%s/graphics/validation/muscle_activations_vs_EMG.png" alt="Muscle Activations vs EMG Plot" style="max-width:100%%; height:auto;">\n', patientID);
        fprintf(fid, '</div>\n');
        ...
        fprintf(fid, '<p>No EMG data available for this patient.</p>\n');
        fprintf(fid, '</div>\n');
    end
